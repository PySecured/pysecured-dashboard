      ...
      <section className="relative max-w-4xl mx-auto px-4 sm:px-10 pt-6 sm:pt-10">
        ...
        <div className="surface rounded-2xl border border-[var(--py-yellow)]/35 p-5 sm:p-6 mb-4">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-5 pt-5 border-t border-[var(--line)]">
            <Link
              to="/servers"
              className="press inline-flex items-center gap-2 rounded-xl btn-primary text-sm font-semibold px-4 py-2.5"
            >
            ...
            <a
              ...
              className="press inline-flex items-center gap-2 rounded-xl btn-secondary text-sm font-medium px-4 py-2.5"
            >
              Get support
            </a>
          </div>
...
export default function Premium() {
  ...
  // Capture a referral code on arrival. Stashed in sessionStorage first
  // because most visitors land here logged out — the claim has to wait
  // until they've signed in, or the referral is simply lost.
  useEffect(() => {
    const ref = new URLSearchParams(window.location.search).get('ref')
    if (ref) {
      try { sessionStorage.setItem('pysecured_ref', ref) } catch { /* ignore */ }
    }
  }, [])
  ...
  useEffect(() => {
    if (!user) return
    let pending = null
    try { pending = sessionStorage.getItem('pysecured_ref') } catch { /* ignore */ }
    if (!pending) return
    api.claimReferral(pending)
      .catch(() => { /* already referred, self-referral, or bad code */ })
      .finally(() => {
        try { sessionStorage.removeItem('pysecured_ref') } catch { /* ignore */ }
      })
  }, [user])
