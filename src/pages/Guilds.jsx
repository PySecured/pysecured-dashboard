...
import { Plus, ChevronRight, ShieldCheck, ShieldOff, Server } from 'lucide-react'
import { api } from '../api'
...
export default function Guilds() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      <main className="relative max-w-3xl mx-auto px-6 sm:px-10 py-10">
        <Reveal>
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <PageHeader
              eyebrow="Servers"
              title="Your servers"
              subtitle="Pick a server to configure. You can manage any server where you have Manage Server permission."
              icon={Server}
            />
            <button
          ...
          {configured.map((g, idx) => (
            <Reveal key={g.id} delay={idx * 60}>
              <button
                ...
                className="card-interactive w-full flex items-center gap-3 surface rounded-2xl border border-[var(--line)] p-4 text-left"
              >
                <GuildIcon guild={g} />
