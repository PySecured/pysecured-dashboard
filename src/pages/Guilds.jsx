    ...
    <div className="min-h-screen overflow-x-hidden">
      ...
      <main className="relative max-w-2xl mx-auto px-6 py-10">
        <Reveal>
          <div className="flex items-start justify-between gap-4 mb-8">
            ...
            <button
              ...
              className="shrink-0 rounded-lg bg-[var(--discord)] hover:brightness-110 disabled:opacity-50 transition-all text-white text-sm font-medium px-4 py-2"
            >
        ...
        {data && configured.length === 0 && (
          <Reveal delay={100}>
            <div className="rounded-xl border border-[var(--line)] bg-[var(--bg-raised)]/60 p-6 text-center mb-8">
          ...
          {configured.map((g, idx) => (
            <Reveal key={g.id} delay={idx * 60}>
              <button
                ...
                className="w-full flex items-center gap-3 rounded-xl border border-[var(--line)] bg-[var(--bg-raised)]/60 hover:bg-[var(--bg-raised)] hover:border-[var(--py-blue)]/40 transition-colors p-4 text-left"
              >
                <GuildIcon guild={g} />
                <span className="text-sm text-white font-medium flex-1">{g.name}</span>
                <span
                  className={`font-mono text-[11px] font-medium px-2 py-1 rounded-full shrink-0 ${
                ...
                {available.map((g) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-lg bg-[var(--bg-raised)] hover:brightness-125 disabled:opacity-50 border border-[var(--line)] transition-all text-white text-xs font-medium px-3 py-1.5"
                    >
