...
export default function Guilds() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      <main className="relative max-w-4xl mx-auto px-4 sm:px-10 pt-8 sm:pt-12 pb-16">
        <Reveal>
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-7">
            <div>
              <p className="eyebrow mb-3">Servers</p>
              <h1 className="font-display text-2xl sm:text-3xl font-bold text-white">Your servers</h1>
              <p className="text-sm text-[var(--mist-dim)] mt-1.5 max-w-xl">
                Pick a server to configure. You can manage any server where you have Manage
                Server permission.
              </p>
            </div>
            <button
              ...
              className="press group shrink-0 inline-flex items-center gap-1.5 rounded-xl bg-[var(--discord)] hover:brightness-110 disabled:opacity-50 transition-all text-white text-sm font-medium px-4 py-2.5"
            >
        ...
        {data && configured.length > 0 && (
          <Reveal delay={50}>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              <div className="surface rounded-[var(--radius-md)] border border-[var(--line)] p-4">
        ...
        {available.length > 0 && (
          <Reveal delay={configured.length * 60 + 100}>
            <div className="mt-10">
              <p className="eyebrow mb-2">Not added yet</p>
              ...
              <div className="space-y-2">
                {available.map((g) => (
                  <div
                    key={g.id}
                    className="w-full flex items-center gap-3 rounded-[var(--radius-md)] border border-[var(--line)] bg-[var(--bg-raised)]/40 p-4"
                  >
                    <GuildIcon guild={g} />
                    ...
                    <button
                      ...
                      className="press btn-secondary group shrink-0 inline-flex items-center gap-1 rounded-lg text-xs font-medium px-3 py-1.5"
                    >
