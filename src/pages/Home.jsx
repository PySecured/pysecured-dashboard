  ...
  return (
    <div className="min-h-screen overflow-x-hidden">
      ...
      <section className="relative overflow-hidden">
        {/* Faint engineering grid — reads as "product", not "poster" */}
        <div
          aria-hidden
        ...
        <div
          aria-hidden
          className="absolute -top-32 left-1/2 -translate-x-1/2 w-[640px] h-[420px] rounded-full bg-[var(--py-blue)] blur-[130px] opacity-[0.18]"
        />

        <div className="relative max-w-3xl mx-auto px-6 pt-20 sm:pt-28 pb-16 text-center">
          <Reveal>
            <span className="inline-flex items-center gap-2 rounded-full border border-[var(--line-strong)] bg-[var(--bg-raised)]/70 pl-1.5 pr-3.5 py-1.5 mb-7">
              <span className="w-4.5 h-4.5 rounded-full bg-[var(--py-blue)]/15 flex items-center justify-center">
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--py-blue)]" />
              </span>
              <span className="font-mono text-[11px] tracking-wide text-[var(--mist)]">
                v1.3 · protecting {status?.bot?.guild_count ? `${status.bot.guild_count.toLocaleString()} servers` : 'thousands of servers'}
              </span>
            </span>
          </Reveal>

          <Reveal delay={60}>
            <h1 className="font-display text-4xl sm:text-6xl font-bold leading-[1.05] tracking-tight text-white mb-6 text-balance">
              The all-in-one bot<br />for Discord
            </h1>
          </Reveal>

          <Reveal delay={120}>
            <p className="text-[var(--mist)] text-base sm:text-lg leading-relaxed mb-9 max-w-lg mx-auto text-balance">
              Security, moderation, tickets, verification, leveling and more — managed from
              one clean dashboard. Almost entirely free.
            </p>
          </Reveal>
            ...
            <div className="flex flex-col items-center gap-3">
              <div className="flex flex-wrap items-center justify-center gap-3">
                {user ? (
                  <Link
                    to="/servers"
                    className="press btn-primary group inline-flex items-center gap-2 rounded-xl text-sm font-semibold px-6 py-3.5"
                  >
                    Open Dashboard
                ...
                <a
                  href="#spotlights"
                  className="press btn-secondary inline-flex items-center gap-2 rounded-xl text-sm font-medium px-6 py-3.5"
                >
                  See what it does
                </a>
              </div>
              <span className="text-xs text-[var(--mist-dim)]">
                {user ? `Logged in as ${user.username}.` : 'No credit card. Manage Server permission to add it.'}
              </span>
            </div>
          </Reveal>
        </div>
      ...
      {status?.bot?.online && (
        <section className="relative max-w-5xl mx-auto px-6 pb-20">
          <Reveal delay={220}>
            <div className="surface rounded-[var(--radius-lg)] border border-[var(--line)] grid grid-cols-2 sm:grid-cols-5 divide-x divide-y sm:divide-y-0 divide-[var(--line)]">
              {[
                [status.bot.guild_count?.toLocaleString?.() ?? status.bot.guild_count, 'servers'],
                [status.bot.member_count?.toLocaleString?.() ?? '—', 'members'],
                ['28', 'features'],
                ['25', 'commands'],
                [`${status.bot.latency_ms} ms`, 'latency'],
              ].map(([v, l], i) => (
                <div key={l} className={`px-4 py-5 text-center ${i === 4 ? 'col-span-2 sm:col-span-1' : ''}`}>
                  <p className="font-display text-xl sm:text-2xl font-bold text-white">{v}</p>
                  <p className="text-[11px] text-[var(--mist-dim)] mt-1 tracking-wide uppercase">{l}</p>
                </div>
              ))}
            </div>
          </Reveal>
      ...
      {/* Spotlights */}
      <section id="spotlights" className="relative max-w-6xl mx-auto px-6 py-14 sm:py-20 space-y-16 sm:space-y-24">
      ...
      <section className="relative max-w-6xl mx-auto px-6 py-14 sm:py-20">
        <Reveal className="card-interactive surface rounded-2xl border border-[var(--line)] px-6 sm:px-8 py-12 sm:py-14 text-center">
          ...
          {user ? (
            <Link
              to="/servers"
              className="press group inline-flex items-center gap-2 rounded-lg btn-primary text-sm font-semibold px-6 py-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--py-blue)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg)]"
            >
              Open Dashboard
              <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-0.5" strokeWidth={2} />
            </Link>
