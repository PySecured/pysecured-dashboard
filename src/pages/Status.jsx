      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-10 pt-8 sm:pt-12 pb-16">
        <div className="flex items-end justify-between gap-4 mb-7">
          <div>
            ...
            <h1 className="font-display text-2xl sm:text-3xl font-bold text-white">System status</h1>
        ...
        {data && (
          <>
            {/* Unified summary card — overall state + the four key numbers
                live together as one surface instead of five separate boxes
                competing for attention. */}
            <div className={`rounded-[var(--radius-lg)] border ${overall.ring} overflow-hidden mb-6`}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-[var(--line)] border-t border-[var(--line)] bg-[var(--bg)]/30">
                {[
                  [Gauge, 'Latency', data.bot.latency_ms != null ? `${data.bot.latency_ms} ms` : '—'],
                  [Users2, 'Servers', data.bot.guild_count ?? '—'],
                  [Users2, 'Members', data.bot.member_count != null ? data.bot.member_count.toLocaleString() : '—'],
                  [Timer, 'Uptime', formatUptime(data.bot.uptime_seconds)],
                ].map(([Icon, label, value]) => (
                  <div key={label} className="px-4 sm:px-5 py-4">
                    <div className="flex items-center gap-1.5 mb-1">
                      <Icon className="w-3.5 h-3.5 text-[var(--mist-dim)]" strokeWidth={2} />
                      <p className="text-xs text-[var(--mist-dim)]">{label}</p>
                    </div>
            ...
            <div className="surface rounded-[var(--radius-lg)] border border-[var(--line)] divide-y divide-[var(--line)] overflow-hidden mb-6">
