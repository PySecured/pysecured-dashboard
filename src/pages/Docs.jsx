...
export default function Docs() {
  ...
  const groupedCommands = COMMAND_GROUPS.map((g) => ({
    ...g,
    items: g.items.filter((c) => matches(c.cmd) || matches(c.desc)),
  })).filter((g) => g.items.length > 0)

  const features = q
    ? FEATURES.filter((f) => matches(f.title) || matches(f.body))
    : FEATURES

  const totalCommands = groupedCommands.reduce((n, g) => n + g.items.length, 0)
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      <div
        aria-hidden
        className="absolute -top-24 -left-32 w-[380px] h-[380px] rounded-full bg-[var(--py-blue)] blur-[130px] opacity-[0.12] pointer-events-none"
      />

      <main className="relative max-w-5xl mx-auto px-4 sm:px-10 pt-8 sm:pt-12 pb-16">
        <Reveal>
          <p className="eyebrow mb-3">Documentation</p>
          <h1 className="font-display text-2xl sm:text-3xl font-bold text-white mb-2">
            Everything PySecured does
          </h1>
          <p className="text-sm text-[var(--mist-dim)] max-w-xl mb-7">
            Every command and feature, what it's for, and where to turn it on. Nothing is
            enabled by default — the bot won't touch your server until you decide.
          </p>

          <div className="relative max-w-lg mb-8">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--mist-dim)]" strokeWidth={2} />
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search commands and features..."
              className="w-full rounded-xl surface border border-[var(--line)] text-sm text-white pl-10 pr-3 py-3 field-focus"
            />
          </div>

          {/* Segmented view toggle — replaces the old always-both-stacked
              layout, so a visitor after one thing isn't scrolling past the
              other to find it. */}
          <div className="inline-flex items-center gap-1 rounded-xl surface border border-[var(--line)] p-1 mb-8">
            {[
        ...
        {view === 'commands' && (
          <Reveal delay={60}>
            <p className="text-sm text-[var(--mist-dim)] mb-6">
              Permissions are Discord's own — fine-tune who can run each command under
              Server Settings → Integrations → PySecured.
            </p>

            {groupedCommands.length === 0 && (
              <p className="text-sm text-[var(--mist-dim)] py-8 text-center">No commands match "{query}".</p>
            )}
              ...
              {groupedCommands.map((g) => (
                <div key={g.label}>
                  <h2 className="font-display text-sm font-bold text-white mb-3 flex items-center gap-2">
                    {g.label}
                    <span className="font-mono text-[10px] font-normal text-[var(--mist-dim)]">
                      {g.items.length}
                    </span>
                  </h2>
                  <div className="surface rounded-[var(--radius-md)] border border-[var(--line)] divide-y divide-[var(--line)] overflow-hidden">
                    {g.items.map((c) => (
                      <div
                        key={c.cmd}
                        className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 px-4 py-3 hover:bg-white/[0.025] transition-colors"
                      >
                        <code className="font-mono text-[13px] text-[var(--py-blue)] sm:w-48 shrink-0">{c.cmd}</code>
                        <span className="text-[13px] text-[var(--mist)] flex-1">{c.desc}</span>
                        <span className="font-mono text-[10px] text-[var(--mist-dim)] border border-[var(--line)] rounded-full px-2 py-0.5 shrink-0 self-start sm:self-auto">
                          {c.perm}
                        </span>
                      </div>
                    ))}
        ...
        {view === 'features' && (
          <Reveal delay={60}>
            {features.length === 0 && (
              <p className="text-sm text-[var(--mist-dim)] py-8 text-center">No features match "{query}".</p>
            )}
            <div className="grid sm:grid-cols-2 gap-3">
              {features.map((f) => (
                <div key={f.title} className="card-interactive surface rounded-[var(--radius-md)] border border-[var(--line)] p-4">
                  <div className="w-9 h-9 rounded-lg bg-[var(--py-blue)]/10 border border-[var(--py-blue)]/25 flex items-center justify-center mb-3">
                  ...
                  <p className="text-[13px] text-[var(--mist)] leading-relaxed mb-3">{f.body}</p>
                  <p className="text-xs text-[var(--mist-dim)] pt-3 border-t border-[var(--line)]">
