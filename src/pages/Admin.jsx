    ...
    >
      <div className="flex flex-wrap items-center gap-2 py-3">
        ...
        <button
          ...
          className="press shrink-0 inline-flex items-center gap-1.5 rounded-lg btn-primary disabled:opacity-40 text-sm font-semibold px-4 py-2"
        >
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-3 pt-3">
        <button
          ...
          className="press inline-flex items-center gap-1.5 rounded-lg btn-primary disabled:opacity-40 text-sm font-semibold px-4 py-2"
        >
        ...
        <button
          ...
          disabled={busy || !data.test_mode}
          className="press inline-flex items-center gap-1.5 rounded-lg btn-secondary disabled:opacity-40 text-sm font-medium px-4 py-2"
        >
          Open to everyone
        </button>
      </div>
      ...
      <div className="pt-5 border-t border-[var(--line)] mt-5">
        ...
        <button
          onClick={runDiagnose}
          disabled={diagBusy}
          className="press inline-flex items-center gap-1.5 rounded-lg btn-secondary disabled:opacity-50 text-sm font-medium px-4 py-2"
        >
              ...
              {editing === c.name && (
                <div className="mt-3 rounded-lg border border-[var(--line)] p-3 space-y-2">
                  ...
                  <button
                    ...
                    disabled={busy === c.name}
                    className="press inline-flex items-center gap-1.5 rounded-lg btn-primary disabled:opacity-50 text-xs font-semibold px-3 py-1.5"
                  >
...
export default function Admin() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-10 pt-8 sm:pt-12 pb-16">
        <div className="mb-7">
          <p className="eyebrow mb-3">Admin</p>
          <h1 className="font-display text-2xl sm:text-3xl font-bold text-white">Admin</h1>
        ...
        <div className="inline-flex gap-1 overflow-x-auto scrollbar-thin surface border border-[var(--line)] rounded-xl p-1 mb-7 max-w-full">
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`shrink-0 inline-flex items-center gap-2 rounded-lg px-3.5 py-2 text-sm font-medium transition-colors ${
                tab === t.id
