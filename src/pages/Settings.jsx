...
const TAB_GROUPS = [
  ...
  },
  {
    ...
    tabs: [
      ...
      { id: 'dms', label: 'DM Notices', Icon: Mail },
...
export default function Settings() {
  ...
  const [giveawayBusy, setGiveawayBusy] = useState(null)
  const [giveawayForm, setGiveawayForm] = useState({ prize: '', channel_id: '', duration_minutes: 60, winner_count: 1, required_role_id: '', blocked_role_id: '', rules: '' })
  ...
  async function viewEntries(id) {
    ...
    setEntriesData(null)
    try {
      setEntriesData(await api.giveawayEntries(guildId, id))
    } catch (e) {
  ...
  async function rerollGiveaway(id) {
    ...
    try {
      await api.rerollGiveaway(guildId, id, true)
      const fresh = await api.giveaways(guildId)
      setGiveaways(fresh)
      if (entriesFor === id) await viewEntries(id)
    } catch (e) {
      setGiveawayError(e.message)
    } finally {
      setGiveawayBusy(null)
    }
  }
  ...
  async function createGiveaway() {
    ...
    try {
      const created = await api.createGiveaway(guildId, {
        ...
        blocked_role_id: giveawayForm.blocked_role_id || null,
        rules: giveawayForm.rules || '',
      })
      setGiveaways((g) => [created, ...(g || [])])
      setGiveawayForm({ prize: '', channel_id: '', duration_minutes: 60, winner_count: 1, required_role_id: '', blocked_role_id: '', rules: '' })
    } catch (e) {
      setGiveawayError(e.message)
    } finally {
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap pb-4 border-b border-[var(--line)] sticky top-0 lg:top-2 z-20 bg-[var(--bg)]/85 backdrop-blur-md pt-3 rounded-b-xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={save}
              disabled={saving}
              className="press inline-flex items-center gap-1.5 rounded-lg btn-primary disabled:opacity-50 text-sm font-semibold px-4 py-2.5 sm:py-1.5"
            >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-8 pt-6 pb-6 grid lg:grid-cols-[220px_1fr] gap-4 lg:gap-8">
                ...
                <>
                  ...
                  <div className="pt-3 flex flex-wrap items-center gap-2">
                    <button
                      ...
                      className="press inline-flex items-center gap-1.5 rounded-lg btn-primary disabled:opacity-50 text-sm font-semibold px-4 py-2"
                    >
                  ...
                  <div className="px-5 pb-5 border-t border-[var(--line)] pt-4 space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-[var(--line)]">
                      ...
                      <button
                        ...
                        disabled={panelBusy === panel.id}
                        className="press inline-flex items-center gap-1.5 rounded-lg btn-primary disabled:opacity-50 text-xs font-semibold px-3 py-2"
                      >
        ...
        {(tab === 'raid_mode' || tab === 'announcements') && !user?.is_premium && (
          <div className="surface rounded-xl border border-[var(--line)] p-8 text-center">
            ...
            <a
              href="/premium"
              className="press inline-block rounded-lg btn-primary text-sm font-semibold px-5 py-2.5"
            >
              Get Premium
            </a>
          </div>
        ...
        {tab === 'dms' && (
          <>
            <Section
              ...
              icon={Mail}
              description="What PySecured tells members directly, rather than only logging it where they'll never see it."
            >
              <Toggle
                label="Moderation actions"
                description="Warn, mute, kick, ban and softban — with the reason."
                checked={config.dm_on_moderation}
                onChange={(v) => set('dm_on_moderation', v)}
              />
              <Toggle
                label="Automatic moderation"
                description="When the spam catcher or auto-mod acts on someone's message."
                checked={config.dm_on_automod}
                onChange={(v) => set('dm_on_automod', v)}
              />
              <Toggle
                label="Giveaway wins"
                description="Winners get a DM as well as the announcement."
                checked={config.dm_on_giveaway_win}
                onChange={(v) => set('dm_on_giveaway_win', v)}
              />
              <Toggle
                label="Verification complete"
                description="Confirms they're verified and which role they were given."
                checked={config.dm_on_verification}
                onChange={(v) => set('dm_on_verification', v)}
              />
            </Section>

            <Section title="What the DM says">
              <Toggle
                label="Include the moderator's name"
                description="Off by default — naming the individual can invite retaliation."
                checked={config.dm_show_moderator}
                onChange={(v) => set('dm_show_moderator', v)}
              />
              <Field label="Appeal link (optional)">
                <TextInput
                  value={config.dm_appeal_url}
                  onChange={(v) => set('dm_appeal_url', v)}
                  placeholder="https://your-server.example/appeal"
                />
              </Field>
              <p className="text-xs text-[var(--mist-dim)] pt-2">
                Members with DMs closed simply won't receive these — the action still goes
                through, and moderators are told when a warning didn't reach its target.
                Automatic-moderation DMs are sent <strong className="text-[var(--mist)]">before</strong>{' '}
                a kick or ban, since afterwards PySecured can no longer reach them.
              </p>
            </Section>
          </>
        )}
          ...
          <>
            <Section title="Create a giveaway" icon={Gift}>
              ...
              <div className="grid grid-cols-1 xs:grid-cols-2 gap-4">
                ...
                <Field label="Excluded role (optional)">
                  <Select
                    value={giveawayForm.blocked_role_id}
                    onChange={(v) => setGiveawayForm((f) => ({ ...f, blocked_role_id: v }))}
                    options={roles.map((r) => ({ value: r.id, label: r.name }))}
                    placeholder="Nobody is excluded"
                  />
                </Field>
              </div>
              <p className="text-xs text-[var(--mist-dim)] pb-2">
                Handy for excluding past winners or staff. Both requirements show on the
                giveaway embed, so nobody clicks only to find out they were never eligible.
              </p>
              <Field label="Rules (optional)">
                <textarea
                  value={giveawayForm.rules}
                  onChange={(e) => setGiveawayForm((f) => ({ ...f, rules: e.target.value }))}
                  placeholder="e.g. Must be in the server for 7 days. One account per person."
                  rows={3}
                  maxLength={1000}
                  className="w-full rounded-lg bg-[var(--bg)] border border-[var(--line)] text-sm text-white px-3 py-2 field-focus resize-none"
                />
                ...
                {giveaways?.map((g) => (
                  <div key={g.id} className="py-3">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        onClick={() => (entriesFor === g.id ? setEntriesFor(null) : viewEntries(g.id))}
                        className="press rounded-lg bg-[var(--bg-raised)] hover:brightness-125 border border-[var(--line)] transition-all text-white text-xs font-medium px-3 py-1.5"
                      >
                        {entriesFor === g.id ? 'Hide' : `Entries (${g.entries.length})`}
                      </button>
                      {g.ended && g.entries.length > 0 && (
                        <button
                          onClick={() => rerollGiveaway(g.id)}
                          disabled={giveawayBusy === g.id}
                          title="Draw a new winner, excluding previous ones"
                          className="press inline-flex items-center gap-1.5 rounded-lg bg-[var(--bg-raised)] hover:brightness-125 border border-[var(--line)] disabled:opacity-50 transition-all text-white text-xs font-medium px-3 py-1.5"
                        >
                          <RotateCcw className="w-3 h-3" strokeWidth={2.25} />
                          {giveawayBusy === g.id ? 'Rerolling…' : 'Reroll'}
                        </button>
                      )}
                      {!g.ended && (
                        <button
                  ...
                  {entriesFor === g.id && (
                    <div className="mt-3 pt-3 border-t border-[var(--line)]">
                      {!entriesData && <p className="text-xs text-[var(--mist-dim)]">Loading entries…</p>}
                      {entriesData && entriesData.entries.length === 0 && (
                        <p className="text-xs text-[var(--mist-dim)]">Nobody has entered yet.</p>
                      )}
                      {entriesData && entriesData.entries.length > 0 && (
                        <>
                          <p className="text-xs text-[var(--mist-dim)] mb-2">
                            {entriesData.total} {entriesData.total === 1 ? 'entry' : 'entries'}
                            {entriesData.left_server > 0 && ` · ${entriesData.left_server} left the server`}
                            {entriesData.draws?.length > 1 && ` · rerolled ${entriesData.draws.length - 1}×`}
                          </p>
                          <div className="max-h-56 overflow-y-auto scrollbar-thin space-y-1 pr-1">
                            {entriesData.entries.map((e) => (
                              <div key={e.user_id} className="flex items-center gap-2 py-1">
                                {e.avatar ? (
                                  <img src={e.avatar} alt="" className="w-5 h-5 rounded-full shrink-0" />
                                ) : (
                                  <div className="w-5 h-5 rounded-full bg-[var(--bg-elevated)] shrink-0" />
                                )}
                                <span className={`text-xs flex-1 truncate ${e.in_server ? 'text-[var(--mist)]' : 'text-[var(--mist-dim)] line-through'}`}>
                                  {e.username || e.user_id}
                                </span>
                                {e.won && (
                                  <span className="font-mono text-[10px] text-[var(--py-yellow)] border border-[var(--py-yellow)]/30 rounded px-1.5 py-0.5 shrink-0">
                                    WINNER
                                  </span>
                                )}
                                {!e.in_server && (
                                  <span className="text-[10px] text-[var(--mist-dim)] shrink-0">left</span>
                                )}
                              </div>
                            ))}
                          </div>
