...
import { ChevronLeft, Save, ShieldAlert, Radar, UserPlus, Users, Ticket, Lock } from 'lucide-react'
import { api } from '../api'
        ...
        <div className="flex gap-1.5 mb-6 overflow-x-auto pb-1">
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`inline-flex items-center gap-1.5 shrink-0 rounded-lg px-3 py-2 text-xs font-medium transition-colors ${
                tab === t.id
                  ? 'bg-[var(--py-blue)] text-[#06111f]'
                  : 'bg-[var(--bg-raised)] border border-[var(--line)] text-[var(--mist-dim)] hover:text-[var(--mist)] hover:border-[var(--py-blue)]/30'
              }`}
            >
              <t.Icon className="w-3.5 h-3.5" strokeWidth={2.25} />
              {t.label}
              {t.id === 'tickets' && !user?.is_premium && <Lock className="w-3 h-3 opacity-70" strokeWidth={2.25} />}
            </button>
          ))}
        </div>
