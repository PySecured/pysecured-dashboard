...
import {
  ...
  ShieldX, Timer, Pin, Undo2, Mail, TerminalSquare,
} from 'lucide-react'
import { api } from '../api'
  ...
  return (
    <div className="min-h-screen overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between gap-3 flex-wrap pb-4 border-b border-[var(--line)] sticky top-0 z-20 bg-[var(--bg)]/80 backdrop-blur-md pt-4 -mt-4">
          <button
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 grid lg:grid-cols-[220px_1fr] gap-4 lg:gap-8">
          ...
          <div className="flex flex-col gap-1.5">
            {TAB_GROUPS.map((group) => (
              <div key={group.label} className="mb-4 last:mb-0">
                <p className="font-mono text-[10px] tracking-[0.15em] text-[var(--mist-dim)] uppercase mb-1.5 px-3">
                  {group.label}
                </p>
                {group.tabs.map((t) => (
                  <button
                    key={t.id}
                    onClick={() => setTab(t.id)}
                    className={`flex items-center gap-2.5 w-full text-left rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${
                      tab === t.id
                        ? 'bg-[var(--py-blue)] text-[#06111f]'
        ...
        {tab === 'member_restore' && !user?.is_premium && (
          <div className="surface rounded-xl border border-[var(--line)] p-8 text-center">
            ...
            <p className="text-sm text-[var(--mist)] max-w-sm mx-auto mb-5">
              ...
              back in automatically. Premium is live now, granted by hand in our Discord.
            </p>
            <a
        ...
        {(tab === 'raid_mode' || tab === 'announcements') && !user?.is_premium && (
          <div className="surface rounded-xl border border-[var(--line)] p-8 text-center">
            ...
            <p className="text-sm text-[var(--mist)] max-w-sm mx-auto mb-5">
              ...
              Premium is live now, granted by hand in our Discord.
            </p>
            <a
              href={DISCORD_SUPPORT_URL}
              target="_blank"
        ...
        {tab === 'identity' && !user?.is_premium && (
          <div className="surface rounded-xl border border-[var(--line)] p-8 text-center">
            ...
            <p className="text-sm text-[var(--mist)] max-w-sm mx-auto mb-5">
              ...
              separate from how it appears anywhere else. Premium is live now, granted by hand in our Discord.
            </p>
            <a
              href={DISCORD_SUPPORT_URL}
              target="_blank"
        ...
        {tab === 'antinuke' && !user?.is_premium && (
          <div className="surface rounded-xl border border-[var(--line)] p-8 text-center">
            ...
            <p className="text-sm text-[var(--mist)] max-w-sm mx-auto mb-5">
              ...
              or mass-banning members. Premium is live now, granted by hand in our Discord.
            </p>
        ...
        {tab === 'temp_roles' && user?.is_premium && (
          <Section title="Temporary roles" icon={Timer} description="Moderators run /temprole to grant a role that expires on its own.">
            ...
            <p className="text-xs text-[var(--mist-dim)] pt-3">
              Usage: <code className="font-mono">/temprole @member @role 60</code> grants it for 60 minutes.
              Expiry is checked every minute and survives restarts. Granting the same role again just
              extends it rather than stacking duplicates.
            </p>
          </Section>
        )}
        ...
        {tab === 'backups' && !user?.is_premium && (
          <div className="surface rounded-xl border border-[var(--line)] p-8 text-center">
            ...
            <p className="text-sm text-[var(--mist)] max-w-sm mx-auto mb-5">
              Snapshot your server's roles and channels, restore them later. Premium is live now, granted by hand in our Discord.
            </p>
            <a
              href={DISCORD_SUPPORT_URL}
              target="_blank"
        ...
        {tab === 'analytics' && !user?.is_premium && (
          <div className="surface rounded-xl border border-[var(--line)] p-8 text-center">
            ...
            <p className="text-sm text-[var(--mist)] max-w-sm mx-auto mb-5">
              ...
              and a live activity feed. Premium is live now, granted by hand in our Discord
