...
import {
  ...
  ChevronLeft, ChevronRight, UserRound, Trophy,
} from 'lucide-react'
import { api } from '../api'
...
export default function SiteSidebar({ children }) {
        ...
        {!loading && user && !mini && (
          <div className="surface rounded-xl border border-[var(--line)] p-3">
            <Link
              to="/profile"
              onClick={onNavigate}
              className="flex items-center gap-2.5 mb-2.5 rounded-lg hover:bg-white/[0.03] -m-1 p-1 transition-colors"
            >
              {user.avatar ? (
  ...
  return (
    <div className="min-h-screen lg:flex">
      {/* Desktop rail */}
      <aside
        className={`relative hidden lg:flex lg:flex-col shrink-0 border-r border-[var(--line)] bg-[var(--bg-raised)]/50 p-4 h-screen sticky top-0 transition-[width] duration-200 ease-out ${
      ...
      {/* Mobile bar */}
      <div className="glass lg:hidden sticky top-0 z-40 flex items-center justify-between px-4 py-3 border-b border-[var(--line)]">
